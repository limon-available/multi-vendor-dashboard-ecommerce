...
const socket = io('http://localhost:5000', { // অথবা তোমার production backend URL
...
const SellerDashboard = () => {
 ...
 useEffect(() => {
  if (userInfo?.id && userInfo.role === 'seller') {
    ...
    // cleanup only on unmount, not immediately
    return () => {
               ...
               
    {
        ...
        {
            m.senderId !== userInfo.id ? <img className='w-full rounded-full h-full shadow-lg' src={userInfo.image} alt="" /> : <img className='w-full rounded-full h-full shadow-lg' src={customer} alt="" />
        } 
        </div>
