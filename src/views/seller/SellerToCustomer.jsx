...
const SellerToCustomer = () => {
  ...
  useEffect(() => {
    if (sellerId) {
      dispatch(get_customers(sellerId));
    }
  }, [sellerId, dispatch]);

  // ✅ get customer messages when customerId changes
  useEffect(() => {
    if (customerId) {
  ...
  const send = (e) => {
    ...
    const payload = {
      ...
      message: text,
      senderName: userInfo?.shopInfo?.shopName || userInfo?.name
    };
  ...
  // ✅ emit message via socket when success
  useEffect(() => {
    if (successMessage) {
  ...
  // ✅ receive message from customer via socket
  useEffect(() => {
  ...
  useEffect(() => {
    if (receverMessage) {
      if (customerId === receverMessage.senderId && sellerId === receverMessage.receverId) {
  ...
  }, [receverMessage, customerId, sellerId, dispatch]);
            ...
            <div className='flex justify-between items-center'>
              {sellerId && (
                <div className='flex justify-start items-center gap-3'>
                  ...
                  <h2 className='text-base text-white font-semibold'>{currentCustomer?.name || "Select Customer"}</h2>
                </div>
              )}
